...
file_path = 'arr_10s.csv'  # file path
try:
...
# Skiping the first row (which contains units) and cleaning column names
try:
    ecg_data_cleaned = ecg_data[1:]  # Skipping the first row&column
    ecg_data_cleaned.columns = ['Elapsed time','ECG I filtered','Ecg 2']  # Renaming columns properly #adjusted the column according to our need
    # print("Data cleaned and columns renamed.")
except Exception as e:
    print(f"Error processing data: {e}")

# Converting the 'ECG I filtered' column to float for analysis
try:
...
# Performing 4-level undecimated wavelet transform (MODWT) using sym4
try:
...
# Recreating an empty list for reconstruction, initializing only d3 and d4 to zero arrays
try:
...
try:
    ...
    # Calculating distances between consecutive peaks (RR intervals)
    rr_intervals = np.diff(peaks)
    print("RR intervals (distances between peaks):", rr_intervals)
    
    # Calculating the mean and standard deviation of RR intervals
    mean_rr = np.mean(rr_intervals)
    std_rr = np.std(rr_intervals)
    
    # Calculating the coefficient of variation (CV) of RR intervals
    cv_rr = (std_rr / mean_rr) * 100
    print(f"Coefficient of Variation (CV) of RR intervals: {cv_rr:.2f}%")
    
    # Defining a threshold for arrhythmia based on CV (this threshold is an example and may vary)
    arrhythmia_threshold = 9  # Example threshold, typically between 10-20%
    
    # Comparing CV to the threshold
    if cv_rr > arrhythmia_threshold:
        print("The patient is arrhythmic.")
    else:
        print("The patient's heart rhythm is normal.")
    
    # Getting peak heights and convert peak locations to integer indices
